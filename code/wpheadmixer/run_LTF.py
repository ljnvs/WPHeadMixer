...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type = str, required = False, choices = ['WPMixer', 'WPMixerKAN'], default = 'WPMixerKAN', help = 'model of experiment')
    parser.add_argument('--head_type', type = str, choices = ['linear', 'mlp', 'kan', 'hybrid'], default = None, help = 'forecast head type; defaults to linear for WPMixer and kan for WPMixerKAN')
    parser.add_argument('--match_head_params', action = 'store_true', default = False, help = 'match head parameter budgets across linear/mlp/kan heads')
    parser.add_argument('--head_param_budget', type = int, default = None, help = 'explicit head parameter budget per branch')
    parser.add_argument('--mlp_hidden_dim', type = int, default = None, help = 'hidden size for MLP head; auto-computed when omitted')
    parser.add_argument('--kan_grid_size', type = int, default = 5, help = 'grid size for KAN head')
    parser.add_argument('--kan_spline_order', type = int, default = 3, help = 'spline order for KAN head')
    parser.add_argument('--hybrid_linear_ratio', type = float, default = 0.5, help = 'fraction of matched head budget assigned to the linear branch inside hybrid head')
    parser.add_argument('--kan_reg_weight', type = float, default = 0.0, help = 'activation regularization weight for KAN head')
    parser.add_argument('--kan_entropy_weight', type = float, default = 0.0, help = 'entropy regularization weight for KAN head')
    parser.add_argument('--task_name', type = str, required = False, choices = ['long_term_forecast'], default = 'long_term_forecast')
    parser.add_argument('--data', type = str, default = 'ETTh1', choices = ['ETTh1', 'ETTh2', 'ETTm1', 'ETTm2', 'Electricity', 'Weather', 'Traffic', 'Exchange'], help = 'dataset')
    ...
    ''' Model Parameters '''
    parser.add_argument('--seq_len', type = int, default = 96, help = 'length of the look back window')
    ...
    if args.head_type is None:
        args.head_type = 'kan' if args.model == 'WPMixerKAN' else 'linear'
    ...
    data_parser = {
        ...
        'Exchange': {'data': 'exchange_rate.csv', 'root_path': './data/exchange_rate/', 'T': 'OT', 'M': [8, 8], 'S': [1, 1], 'MS': [8, 1]},
    ...
    if args.use_hyperParam_optim == False: 
        ...
        setting = '{}_{}_head-{}_dec-{}_sl{}_pl{}_dm{}_bt{}_wv{}_tf{}_df{}_ptl{}_stl{}_sd{}'.format(args.model, args.data, args.head_type, not args.no_decomposition, args.seq_len, args.pred_len, args.d_model, args.batch_size, args.wavelet, args.tfactor, args.dfactor, args.patch_len, args.stride, args.seed)
